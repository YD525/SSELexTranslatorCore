...
using SSELexTranslatorCore.RequestManagement;
...
namespace SSELexTranslatorCore.PlatformManagement
{
    ...
    public class GeminiApi
    {
        ...
        public string QuickTrans(string TransSource, Languages FromLang, Languages ToLang, bool UseAIMemory, int AIMemoryCountLimit, string Param)
        {
            ...
            if (Translator.CurrentSetting.UsingContext && UseAIMemory)
            {
            ...
            if (ConvertHelper.ObjToStr(Translator.CurrentSetting.UserCustomAIPrompt).Trim().Length>0)
            {
                GetTransSource += Translator.CurrentSetting.UserCustomAIPrompt + "\n\n";
            }
        ...
        public GeminiRootobject CallAI(GeminiItem Item)
        {
            ...
            {
                URL = $"https://generativelanguage.googleapis.com/v1beta/models/{Translator.CurrentSetting.GeminiModel}:generateContent?key={Translator.CurrentSetting.GeminiKey}",
            ...
            try
            {
                DelegateHelper.SetLog("Gemini:" + GetResult);
